...
@section('content')
    <div class="col-sm-12  justify-content">
        <div class="bg-light rounded h-100 p-4">
            ...
            <h4 class="mb-4">Cập nhật User</h4>

            @if (Session::has('msg'))
                <div class="alert alert-success" role="alert">{{ Session::get('msg') }}</div>
            @endif

            <form action="{{ route('update.user.admin', ['id' => $user->id]) }}" method="post" enctype="multipart/form-data">
                @csrf
                ...
                <button type="submit" class="btn btn-primary mt-3">Cập nhật User</button>
            </form>
        </div>
    </div>
@endsection
