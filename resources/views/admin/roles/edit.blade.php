...
@section('content')
    <div class="col-sm-12  justify-content">
        <div class="bg-light rounded h-100 p-4">
            ...
            <h4 class="mb-4">Update Vai trò</h4>

            @if (Session::has('msg'))
                <div class="alert alert-success" role="alert">{{ Session::get('msg') }}</div>
            @endif

            <form action="{{ route('update.role.admin', ['id' => $role->id]) }}" method="post" enctype="multipart/form-data">
                @csrf
                ...
                <div class="mt-3">
                    ...
                    <textarea class="form-control" name="display_name" id="content" rows="3">{{ $role->display_name }}</textarea>
                    @error('description')
                        <span class="text-danger"><strong>{{ $message }}</strong></span>
                    @enderror
                </div>
    ...
    <button type="submit" class="btn btn-primary mt-3">Update vai trò</button>
    </form>
    </div>
    </div>
@endsection
